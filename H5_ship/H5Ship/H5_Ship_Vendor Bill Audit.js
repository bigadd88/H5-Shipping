function generateBillSelector() {
    ...
    if (request.getMethod() == 'GET') {
        ...
        //form buttons
        form.setScript('customscript_h5_vendor_bill_audit_cs');
        form.addButton('submitForm', 'Submit', 'SubmitForm()');
      	//Add the search column names to the sublist field
        sublist.addField('custrecord_h5_bill_selected', 'checkbox', 'Selected');
        ...
        sublist.addField('custpage_h5_carrierbilledamount', 'CURRENCY', 'Carrier Billed Amount');
        sublist.addField('amount', 'CURRENCY', 'Amount');
      	sublist.addField('custpage_h5_difference', 'CURRENCY', 'Difference');
      	//sublist.setOrder('entity');
        //run existing saved search
        var fil = new Array();
        //fil[0] = new nlobjSearchFilter('type', null, 'is', 'bill');
        fil[0] = new nlobjSearchFilter('custbody_h5_audit_date', null, 'isempty');
        var col = new Array();
        ...
        col[6] = new nlobjSearchColumn('internalid');
      	col[7] = new nlobjSearchColumn('custbody_h5_carrierbilledamount');
        var searchResults = nlapiSearchRecord('transaction', 'customsearch_h5_vendor_bill_audit', fil, col);
        //additional sublist fields
        for (var i = 0; i < searchResults.length; i++){
          	var viewUrl = nsurl + nlapiResolveURL('RECORD', 'vendorbill', searchResults[i].getId(), 'VIEW');
          	var billedAmount = searchResults[i].getValue('custbody_h5_carrierbilledamount');
            ...
            sublist.setLineItemValue('approvalstatus', row, searchResults[i].getText('approvalstatus'));
          	sublist.setLineItemValue('custpage_h5_carrierbilledamount', row, searchResults[i].getValue('custbody_h5_carrierbilledamount'));
            sublist.setLineItemValue('amount', row, searchResults[i].getValue('amount'));
          	sublist.setLineItemValue('custpage_h5_difference', row, billedAmount - calcAmount);
        }
    ...
    if (request.getMethod() == 'POST') {
        ...
        for (var i = 1; i < req + 1; i++) {
            var vals = request.getLineItemValue('custpage_sublist_id', 'custrecord_h5_bill_selected', i);
            if (vals == 'T') {
                var intId = request.getLineItemValue('custpage_sublist_id', 'internalid', i);
              	...
              	recordLine.setFieldValue('custbody_h5_audit_date', tNow);
              	recordLine.setFieldValue('custbody_h5_status', 4);
