...
function getPRO(){   //testing pro generation api
	...
	var carrierEnt = parentShipment.getFieldValue('custrecord_h5_carrier');
	var selSCAC = nlapiLookupField('vendor', carrierEnt, 'custentity_h5_scac');
	var jPayload = {
...
// 	var url = nlapiResolveURL('SUITELET','customscript_h5_package_form','customdeploy_h5_package_form');
// 	url += '&reqURL=' + reqURL;
...
function vgmForm() {
	var recName = nlapiGetRecordId();
	var url = nlapiResolveURL('SUITELET','customscript_h5_vgm','customdeploy_h5_vgm');
	url += '&recName=' + recName,
		window.open(url);
}
...
function sliForm() {
	var recName = nlapiGetRecordId();
	var url = nlapiResolveURL('SUITELET','customscript_h5_sli','customdeploy_h5_sli');
	url += '&recName=' + recName,
		window.open(url);
}
...
function intlCommercialInvoice() {
	var recName = nlapiGetRecordId();
	var url = nlapiResolveURL('SUITELET','customscript_h5_intlcommercialinvoice','customdeploy_h5_intlcommercialinvoice');
	url += '&recName=' + recName,
		window.open(url);
}
...
function preRateHUD(){
	...
	var destZip = shipRec.getFieldValue('custrecord_h5_shipper_zip');
	var orgZip = shipRec.getFieldValue('custrecord_h5_consignee_zip');
	var shipWeight = shipRec.getFieldValue('custrecord_h5_total_weight');
	var shipClass = '60';
	var palletCount = shipRec.getFieldValue('custrecord_h5_total_packages');
...
function insertHTML(form, rateObj){
	...
	form.setFieldValues({custpage_h5_il_html : totHTML});
	nlapiLogExecution('DEBUG', 'We set HTML inline', 'Success!');
}

function AddixsButton(form) {
	form.setScript('customscript_h5_rate_shipment_bttn');
	form.addButton('custpage_ixsrate', 'IXS Rates', 'getRLRates()');
}
function getRLRates() {
	var recName = nlapiGetRecordId();
	var batId = Date.now();
	var url = nlapiResolveURL('SUITELET','customscript_h5_ship_carrier_rates_rl','customdeploy_h5_ship_carrier_rates_rl');                                                      
	url += '&reqId=' + recName;
	url += '&batchId=' + batId;
	setTimeout(nlapiRequestURL(url), 100);
	var listUrl = nlapiResolveURL('SUITELET','customscript_h5_rate_selector_ss1','customdeploy_h5_rate_selector_ss1');
...
function callCHRRater() {
	var recId = nlapiGetRecordId();
	var batId = Date.now();
	var rateReqURL = nlapiResolveURL('SUITELET','customscript_h5_chr_retriever','customdeploy_h5_chr_retriever');
...
function AddP44RatesButton(form) {
	form.setScript('customscript_h5_rate_shipment_bttn');
	form.addButton('custpage_p44rate', 'Contract Rates', 'callP44Rater()');
}
function callP44Rater() {
	var recId = nlapiGetRecordId();
	var batId = Date.now();
	var rateReqURL = nlapiResolveURL('SUITELET','customscript_h5_p44_retriever','customdeploy_h5_p44_retriever');
	rateReqURL += '&requrl=' + recId;
	rateReqURL += '&batid=' + batId;
	var reqObj = nlapiRequestURL(rateReqURL);
	var respObj = reqObj.getBody();
...
function AddPrintBOLButton(form) {
	form.setScript('customdeploy_h5_rate_shipment_bttn');
...
function printBOL() {
	...
	var url = nlapiResolveURL('SUITELET','customscript_h5_bol','customdeploy_h5_bol');
	url += '&recName=' + recName,
		window.open(url);
}
...
function quickRate(){
	...
	var url = nlapiResolveURL('SUITELET', 'customscript_h5_quick_rate_form', 'customdeploy_h5_quick_rate_form');
...
function AddEmailBrokerButton(form) {
	form.setScript('customdeploy_h5_req_rates');
...
function emailBroker() {
	...
	for ( var i = 0;i < results.length; i++){
		...
		var record = nlapiLoadRecord('customrecord_h5_shipment', shipRecId, null, null);
		...
		var baseURL = 'https://forms.netsuite.com/app/site/crm/externalcustrecordpage.nl?compid=TSTDRV1555031&formid=5&h=AACffht_C67TfdIDHCHtABFuFrbixLo60rg&custrecord_h5_bid2shipment_link=' + encodeURIComponent(shipRecId) + '&custrecord_h5_bid_contactname=' + encodeURIComponent(person[i]) + '&custrecord_h5_bid_contactphone=' + encodeURIComponent(phoneNum[i]) + '&custrecord_h5_bid_contactemail=' + encodeURIComponent(emailAddr[i])+ '&custrecord_h5_broker_vendorid=' + encodeURIComponent(brokerCompany[i]);
		...
		var splitSIMS = record.getFieldValue('custrecord_h5_special_instructions');
		if (splitSIMS === null){splitSIMS = '';}
		var ownerId = record.getFieldValue('custrecord_h5_custservteamlead');
		//var selAccNamesPrnt = '';
		var selAccessorials = record.getFieldValue('custrecord_h5_accessorials');
		...
		col[7] = new nlobjSearchColumn('custrecord_h5_freight_class_value');
		col[8] = new nlobjSearchColumn('custrecord_h5_piece_count');
		col[9] = new nlobjSearchColumn('custrecord_h5_length');
		col[10] = new nlobjSearchColumn('custrecord_h5_width');
		col[11] = new nlobjSearchColumn('custrecord_h5_height');
		var packages = nlapiSearchRecord('customrecord_h5_shipment_line', null, fil, col);
		for (var x = 0; x < packages.length; x++){
			pkgcount.push(packages[x].getValue('custrecord_h5_piece_count'));
			var linHazmat = packages[x].getValue('custrecord_h5_hazmat');
			var linPicCount = packages[x].getValue('custrecord_h5_nmfc_number');
			var linType = packages[x].getText('custrecord_h5_packagetype');
			var linNum = packages[x].getValue('custrecord_h5_piece_count');
			var linDesc = packages[x].getValue('custrecord_h5_ship_line_item_desc');
			var linWeight = packages[x].getValue('custrecord_h5_weight');
			var linLength = packages[x].getValue('custrecord_h5_length');
			var linWidth = packages[x].getValue('custrecord_h5_Width');
			var linHeight = packages[x].getValue('custrecord_h5_Height');
			var linNMFC = packages[x].getValue('custrecord_h5_freight_class_value');
		...
		var rec = new Array();
		rec['recordtype']='customrecord_h5_shipment'; //script id of the custom record type
	...
	record.setFieldValue('custrecord_h5_shipment_status',7);
...
function emailBroker1218old() {
	...
	for ( var i = 0;i < results.length; i++){
		...
		//window.alert("Shipment number " + shipRecId + " has been loaded");
		var recNum = record.getFieldValue('name');
		var locationId = record.getFieldValue('custrecord_h5_location');
		//var locationName = record.getFieldText('custrecord_h5_location');
		...
		var emailSubject = "Please quote load #" + recNum;
		var baseURL = 'https://forms.na3.netsuite.com/app/site/crm/externalcustrecordpage.nl?compid=3496142&formid=6&h=AACffht_-ttRtcS_t3VThFzb6cshX56j1-k&redirect_count=1&did_javascript_redirect=T&custrecord_h5_bid2shipment_link=' + encodeURIComponent(shipRecId) + '&custrecord_h5_bid_contactname=' + encodeURIComponent(person[i]) + '&custrecord_h5_bid_contactphone=' + encodeURIComponent(phoneNum[i]) + '&custrecord_h5_bid_contactemail=' + encodeURIComponent(emailAddr[i]);
		//var ShipperEnt = record.getFieldValue('custrecord_h5_shipper');
		//var ShipperEntType = nlapiLookupField('entity', ShipperEnt, 'type');
		//if (ShipperEntType === 'CustJob'){ShipperEntType = 'customer';}
		var ConsEnt = record.getFieldValue('custrecord_h5_consignee');
		var ConsType = nlapiLookupField('entity', ConsEnt, 'type');
		if (ConsType === 'CustJob'){ConsType = 'customer';}
		var BillToEnt = record.getFieldValue('custrecord_h5_freight_bill_to');
		var BillToType = nlapiLookupField('entity', BillToEnt, 'type');
		if (BillToType === 'CustJob'){BillToType = 'customer';}
		//var carrierEnt = record.getFieldValue('custrecord_h5_carrier');
		...
		var refatdelivery = record.getFieldValue('custrecord_h5_reference_delivery');
		if (refatdelivery === null){refatdelivery = '';}
		//var carQuoteNum = record.getFieldValue('custrecord_h5_sel_car_quote_num');
		...
		var consigneeAtten = record.getFieldValue('custrecord_h5_consignee_attn');
		if (consigneeAtten === null){consigneeAtten = '';}
		var consigneeAddree = record.getFieldValue('custrecord_h5_consignee_addree');
...
function print4x6Label(){
	var recName = nlapiGetRecordId();
	var url = nlapiResolveURL('SUITELET', 'customscript_h5_4x6label', 'customdeploy_h5_4x6label');
	url += '&recName=' + recName,
		window.open(url);
}
...
function dispatchShipment(){
	...
	var shipperAddressee = parentShipment.getFieldValue('custrecord_h5_shipper_addressee');
	var shipperAddr1 = parentShipment.getFieldValue('custrecord_h5_shipper_addr_1');
	var shipperAddr2 = parentShipment.getFieldValue('custrecord_h5_shipper_addr_2');
	...
	var consigneeAttn = parentShipment.getFieldValue('custrecord_h5_consignee_attn');
	var consigneeAddr1 = parentShipment.getFieldValue('custrecord_h5_consignee_addr1');
	var consigneeAddr2 = parentShipment.getFieldValue('custrecord_h5_consignee_addr2');
	...
	var selSCAC = nlapiLookupField('vendor', carrierEnt, 'custentity_h5_scac');
	var shipDate = parentShipment.getFieldValue('custrecord_h5_ship_date');
	var deliveryDate = parentShipment.getFieldValue('custrecord_h5_delivery_date');
	...
	var packageLength = [];
	var packageWidth = [];
	for (var j = 1; j <= parentShipment.getLineItemCount('recmachcustrecord_h5_shipment_parent'); j++){
		packageItem.push(parentShipment.getLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_ship_line_item_desc', j));
		packageQty.push(parentShipment.getLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_piece_count', j));
		packageFrtClass.push(parentShipment.getLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_freight_class_value', j));
		packageWeight.push(parentShipment.getLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_weight', j));
		packageHeight.push(parentShipment.getLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_height', j));
		packageLength.push(parentShipment.getLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_length', j));
		packageWidth.push(parentShipment.getLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_width', j));
	}
	...
	if (respObj.hasOwnProperty('httpStatusCode')){
		...
		parentShipment.setFieldValue('custrecord_h5_shipment_status', 1);
		parentShipment.setFieldValue('custrecord_h5_car_pu_num', pickupNum);
...
function cancelDispatch(){
	...
	var shipmentDate = parentShipment.getFieldValue('custrecord_h5_ship_date');
	var origPickupNum = parentShipment.getFieldValue('custrecord_h5_car_pu_num');
	var carrierEnt = parentShipment.getFieldValue('custrecord_h5_carrier');
	var carrierName = nlapiLookupField('vendor',carrierEnt,'companyname');
	var carrierPhone = nlapiLookupField('vendor',carrierEnt,'phone');
	var selSCAC = nlapiLookupField('vendor', carrierEnt, 'custentity_h5_scac');
...
function postDAT() {
	...
	var parentShipment = nlapiLoadRecord('customrecord_h5_shipment', recId);
	var shipmentName = parentShipment.getFieldValue('name');
	var employeeId = parentShipment.getFieldValue('custrecord_h5_custservteamlead');
	var primaryDAT = nlapiLookupField('employee', employeeId, 'custentity_h5_dat_primary_key');
	var secondaryDAT = nlapiLookupField('employee', employeeId, 'custentity_h5_dat_secondary_key');
	var equipTypeId = parentShipment.getFieldValue('custrecord_h5_equip_type');
	var equipType = nlapiLookupField('customrecord_h5_equip_types',equipTypeId,'custrecord_h5_equiptype_datid');
	var fullOrPartial = parentShipment.getFieldValue('custrecord_h5_dat_fullorpartial');
	if (fullOrPartial === "1") {fullOrPartial = 'false';} else {fullOrPartial = 'true';}
	var lengthInFeet = parentShipment.getFieldValue('custrecord_h5_dat_length');
	var weight = parentShipment.getFieldValue('custrecord_h5_dat_weight');
	var originCity = parentShipment.getFieldValue('custrecord_h5_shipper_city');
	var originState = parentShipment.getFieldValue('custrecord_h5_shipper_state');
	...
	var comments1 = parentShipment.getFieldValue('custrecord_h5_dat_comments1');
	var comments2 = parentShipment.getFieldValue('custrecord_h5_dat_comments2');
	...
	parentShipment.setFieldValue('custrecord_h5_dat_asset_id', assetId);
	parentShipment.setFieldValue('custrecord_h5_dat_is_posted', "T");
	parentShipment.setFieldValue('custrecord_h5_shipment_status', "8");
	nlapiSubmitRecord(parentShipment);
...
function updateDAT() {
	...
	var parentShipment = nlapiLoadRecord('customrecord_h5_shipment', recId);
	var shipmentName = parentShipment.getFieldValue('name');
	var assetId = parentShipment.getFieldValue('custrecord_h5_dat_asset_id');
	var employeeId = parentShipment.getFieldValue('custrecord_h5_custservteamlead');
	var primaryDAT = nlapiLookupField('employee', employeeId, 'custentity_h5_dat_primary_key');
	var secondaryDAT = nlapiLookupField('employee', employeeId, 'custentity_h5_dat_secondary_key');
	var equipTypeId = parentShipment.getFieldValue('custrecord_h5_equip_type');
...
function deleteDAT() {
	...
	parentShipment.setFieldValue('custrecord_h5_dat_asset_id', '');
	parentShipment.setFieldValue('custrecord_h5_dat_is_posted', "F");
	nlapiSubmitRecord(parentShipment);
...
function h5ShipGenInit(){
	...
	//}
	//else {
	var shipRecURL = nlapiResolveURL('SUITELET', 'customscript_h5_ship_rec_generator', 'customdeploy_h5_ship_rec_generator');
	...
	//var baseURL = 'https://system.netsuite.com/app/common/custom/custrecordentry.nl?rectype=15&id=';
	var baseURL = 'https://tstdrv2251194.app.netsuite.com/app/common/custom/custrecordentry.nl?rectype=741&id='; //added 3/17/202 - addison
	//var baseURL = 'https://tstdrv1555013.app.netsuite.com/app/common/custom/custrecordentry.nl?rectype=59&id=';
...
function getRatesP44() {
	...
	var parentShipment = nlapiLoadRecord('customrecord_h5_shipment', recId);
	var originZip = parentShipment.getFieldValue('custrecord_h5_consignee_zip');
	var destinationZip = parentShipment.getFieldValue('custrecord_h5_shipper_zip');
...
function getSassy(batId){
	...
	for(x=0; x < scacCodes.length; x++){
		...
		rateLine.setFieldValue('custrecord_h5_ratepass_id', x + 1);
		rateLine.setFieldValue('custrecord_h5_scac', scacCodes[x]);
		rateLine.setFieldValue('custrecord_h5_total_shipment_cost', shipRate[x]);
		rateLine.setFieldValue('custrecord_h5_transit_days', transDays[x]);
		rateLine.setFieldValue('custrecord_h5_ratepass_carrier', carriername[x]);
...
function clientFieldChanged(type, name, linenum){
	...
	if(type == 'recmachcustrecord_h5_shipment_parent'){ //checks if the field changed is under sublist 'Lines'
		if(name == 'custrecord_h5_height'){ //checks if the field changed is 'Account'
			var index = nlapiGetCurrentLineItemIndex('recmachcustrecord_h5_shipment_parent');
			console.log('Current line index: ' + index);
			var weight = nlapiGetCurrentLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_weight');
			console.log('Weight is: ' + weight);
			var len = nlapiGetCurrentLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_length');
			console.log('Length is: ' + len);
			var wid = nlapiGetCurrentLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_width');
			console.log('Width is: ' + wid);
			var hgt = nlapiGetCurrentLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_height');
			...
			nlapiSetCurrentLineItemValue('recmachcustrecord_h5_shipment_parent', 'custrecord_h5_pcf', finalPCF);
			//nlapiCommitLineItem('recmachcustrecord_h5_shipment_parent');
			return true;
		}
	}
	...
	if(name == 'shipmethod' && nlapiGetFieldValue('shipmethod') == '817'){
		...
		var respLTLRate = comboSaaS();
		console.log('Rate Returned is: ' + respLTLRate);
		nlapiSetFieldValue('custbody_h5_parcel_cost', respParcRate);
		nlapiSetFieldValue('custbody_h5_ltl_cost', respLTLRate);
	}
	else return true;
}
...
function shipmentOnSave(){
	...
	var shipperAddress = shipment.getFieldValue('custrecord_h5_shipper_addr_1');
	var shipperCity = shipment.getFieldValue('custrecord_h5_shipper_city');
	var shipperState = shipment.getFieldValue('custrecord_h5_shipper_state');
	...
	var consigneeAddress = shipment.getFieldValue('custrecord_h5_consignee_addr_1');
	var consigneeCity = shipment.getFieldValue('custrecord_h5_consignee_city');
	var consigneeState = shipment.getFieldValue('custrecord_h5_consignee_state');
	...
	shipment.setFieldValue('custrecord_h5_shipper_lat', shipperLat);
	shipment.setFieldValue('custrecord_h5_shipper_lng', shipperLng);
	shipment.setFieldValue('custrecord_h5_consignee_lat', consigneeLat);
	shipment.setFieldValue('custrecord_h5_consignee_lng', consigneeLng);
...
function dispatchCerasisShipment(){
	...
	var shipRec = nlapiLoadRecord('customrecord_h5_shipment', recName, null, null);
	shipRec.setFieldValue('custrecord_h5_shipment_status', 3);
	shipRec.setFieldValue('custrecord_h5_carrier_pro', shipRecProNum);
	shipRec.setFieldValue('custrecord_h5_shiprec_cer_conf_rate', shipRecRate);
	shipRec.setFieldValue('custrecord_h5_shiprec_cer_bill_num', shipRecBillNum);
	shipRec.setFieldValue('custrecord_h5_shiprec_cer_bol_str', shipRecBOL);
...
function getCerasisRates(batId){
	...
	for(x=0;x < scacCodes.length; x++){
		...
		rateLine.setFieldValue('custrecord_h5_scac', scacCodes[x]);
		rateLine.setFieldValue('custrecord_h5_total_shipment_cost', shipRate[x]);
		rateLine.setFieldValue('custrecord_h5_ratepass_total_cost', shipRate[x]);
		rateLine.setFieldValue('custrecord_h5_transit_days', transDays[x]);
		rateLine.setFieldValue('custrecord_h5_ratepass_carrier', carriername[x]);
		rateLine.setFieldValue('custrecord_h5_ratepass_guar_cost', guarprice[x]);
		rateLine.setFieldValue('custrecord_h5_ratepass_guar_time', guartime[x]);
		rateLine.setFieldValue('custrecord_h5_lane_name', 'Cerasis');
		rateLine.setFieldValue('custrecord_h5_ratepass_bt_ent', btEnt);
		nlapiSubmitRecord(rateLine);
	}
	return rateLine;
}
function cerasisRetriever(batId) {
	...
	var locCerAcctNum = nlapiLookupField('location', usrOrigLoc, 'custrecord_h5_cer_acct_num');
	var locCerUserName = nlapiLookupField('location', usrOrigLoc, 'custrecord_h5_cer_user_name');
	var locCerPassword = nlapiLookupField('location', usrOrigLoc, 'custrecord_h5_cer_pw');
	var locCerAccKey = nlapiLookupField('location', usrOrigLoc, 'custrecord_h5_cer_acc_key');
	...
	var parentShipment = nlapiLoadRecord('customrecord_h5_shipment', recName);
	var pkgcount = [];
	var pkgType = [];
	var linQty = [];
	var linNMFC = [];
	...
	var totWeight = 0;
	var fil = [];
	fil[0] = new nlobjSearchFilter('custrecord_h5_shipment_parent', null, 'is', recName);
	var col = [];
	col[0] = new nlobjSearchColumn('custrecord_h5_hazmat');
	col[1] = new nlobjSearchColumn('custrecord_h5_nmfc_number');
	...
	col[4] = new nlobjSearchColumn('custrecord_h5_pkgnumber');
	col[5] = new nlobjSearchColumn('custrecord_h5_weight');
	col[6] = new nlobjSearchColumn('custrecord_h5_height');
	col[7] = new nlobjSearchColumn('custrecord_h5_width');
	col[8] = new nlobjSearchColumn('custrecord_h5_length');
	col[9] = new nlobjSearchColumn('custrecord_h5_nmfc_number_line');
	col[10] = new nlobjSearchColumn('custrecord_h5_inventoryunits');
	var packages = nlapiSearchRecord('customrecord_h5_shipment_line', null, fil, col);
	for (var x = 0; x < packages.length; x++) {
		pkgcount.push(packages[x].getValue('custrecord_h5_pkgnumber'));
		pkgType.push(packages[x].getText('custrecord_h5_packagetype'));
		linWeight.push(Number(packages[x].getValue('custrecord_h5_weight')));
		linHeight.push(Number(packages[x].getValue('custrecord_h5_height')));
		linWidth.push(Number(packages[x].getValue('custrecord_h5_width')));
		linLeng.push(Number(packages[x].getValue('custrecord_h5_length')));
		linFrClass.push(packages[x].getValue('custrecord_h5_freight_class_value'));
		linNMFC.push(packages[x].getValue('custrecord_h5_nmfc_number'));
		linQty.push(Number(packages[x].getValue('custrecord_h5_inventoryunits')));
	}
	var totWeight = linWeight.reduce(function(acc, val) { return acc + val; });
	var pkgcnt = Math.max.apply(null, pkgcount);
	console.log('Package Types: ' + pkgType);
	var message = '';
	var origZip = parentShipment.getFieldValue('custrecord_h5_shipper_zip');
	var consigneeZip = parentShipment.getFieldValue('custrecord_h5_consignee_zip');
	var shipmentDate = parentShipment.getFieldValue('custrecord_h5_ship_date');
	//collect destination info
	var consignee = parentShipment.getFieldValue('custrecord_h5_consignee');
	var consigneeEmail = 'robert.regnier@priority-logistics.com';
	var consigneeAddree = parentShipment.getFieldValue('custrecord_h5_consignee_addree');
	var consigneeAddr1 = parentShipment.getFieldValue('custrecord_h5_consignee_addr1');
	var consigneeAddr2 = parentShipment.getFieldValue('custrecord_h5_consignee_addr2');
	var consigneeCity = parentShipment.getFieldValue('custrecord_h5_consignee_city');
	var consigneeState = parentShipment.getFieldValue('custrecord_h5_consignee_state');
...
function cerProcShipRec(batId) {
	...
	var usrOrigLoc = usrCont.location;
	var usrOrigZip = nlapiLookupField('location', usrOrigLoc, 'zip');
	var uOCerID = nlapiLookupField('location', usrOrigLoc, 'custrecord_h5_cer_acct_num');
	var uOCerUN = nlapiLookupField('location', usrOrigLoc, 'custrecord_h5_cer_user_name');
	var uOCerPW = nlapiLookupField('location', usrOrigLoc, 'custrecord_h5_cer_pw');
	var uOCerAcc = nlapiLookupField('location', usrOrigLoc, 'custrecord_h5_cer_acc_key');
	var uOrigAddr1 = nlapiLookupField('location', usrOrigLoc, 'address1');
	var uOrigAddr2 = nlapiLookupField('location', usrOrigLoc, 'address2');
	var uOrigCity = nlapiLookupField('location', usrOrigLoc, 'city');
	var uOrigState = nlapiLookupField('location', usrOrigLoc, 'state');
	...
	col[9] = new nlobjSearchColumn('custrecord_h5_nmfc_number_line');
	col[10] = new nlobjSearchColumn('custrecord_h5_inventoryunits');
	col[11] = new nlobjSearchColumn('custrecord_h5_piece_count');
	var packages = nlapiSearchRecord('customrecord_h5_shipment_line', null, fil, col);
	for (var x = 0; x < packages.length; x++) {
		pkgcount.push(packages[x].getValue('custrecord_h5_pkgnumber'));
		pkgType.push(packages[x].getText('custrecord_h5_packagetype'));
		linDesc.push(packages[x].getText('custrecord_h5_nmfc_number_line'));
		linNmfcId.push(packages[x].getValue('custrecord_h5_nmfc_number_line'))
		linWeight.push(Number(packages[x].getValue('custrecord_h5_weight')));
		linHeight.push(Number(packages[x].getValue('custrecord_h5_height')));
		linWidth.push(Number(packages[x].getValue('custrecord_h5_width')));
		linLeng.push(Number(packages[x].getValue('custrecord_h5_length')));
		...
		linNMFC.push(packages[x].getValue('custrecord_h5_nmfc_number'));
		linQty.push(Number(packages[x].getValue('custrecord_h5_inventoryunits')));
		//linDesc.push(packages[x].getValue('custrecord_h5_piece_count'));
	}
	var totWeight = linWeight.reduce(function(acc, val) { return acc + val; });
	var pkgcnt = Math.max.apply(null, pkgcount);
	for (var z = 0; z < linNmfcId.length; z++){
		isDimmed.push(nlapiLookupField('customrecord_h5_nmfc_classification', linNmfcId[z], 'custrecord_h5_dim_item'));
	}
	var message = '';
	var origZip = parentShipment.getFieldValue('custrecord_h5_shipper_zip');
	var destZip = parentShipment.getFieldValue('custrecord_h5_consignee_zip');
	var shipmentDate = parentShipment.getFieldValue('custrecord_h5_ship_date');
	var custPoRef = parentShipment.getFieldValue('custrecord_h5__reference_delivery');
	//collect destination info
	var destParent = parentShipment.getFieldValue('custrecord_h5_so_parent_id');
	...
	var formDate = new Date(shipmentDate).toISOString();
	var carrierSCAC = parentShipment.getFieldValue('custrecord_h5_sel_scac_code');
	var specInst = parentShipment.getFieldValue('custrecord_h5_special_instructions');
...
function printCerBOL(){
	var recName = nlapiGetRecordId();
	//var recType = 'customrecord_h5_shipment';
	//var shipRec = nlapiLoadRecord(recType,recName);
	//var shipRecBOL = shipRec.getFieldValue('custrecord_h5_shiprec_cer_bol_str');
	var rcbURL = nlapiResolveURL('SUITELET', 'customscript_h5_render_cer_bol_sl','customdeploy_h5_render_cer_bol_sl');
...
function RenderBOL(request, response) {
	if (request.getMethod() == 'GET') {
		//var recId = request.getParameter('NLNAME');
		var recId = nlapiGetRecordId();
		var rec = nlapiLoadRecord('customrecord_h5_shipment', recId, null, null);
		var bolObj = rec.getFieldValue('custrecord_h5_shiprec_cer_bol_str');
		//var file = nlapiLoadFile(106);
		//var contents = file.getValue();
...
function GenerateAWSResponse(response) {
	...
	var rec = nlapiGetNewRecord('customrecord_h5_response_repo');
	var loadRec = nlapiCreateRecord('customrecord_h5_response_repo', rec.getId());
	loadRec.setFieldValue('custrecord_h5_rr_shipmentid', 'AJRTestAWSPost');
	loadRec.setFieldValue('custrecord_h5_rr_message', json);
...
function generateVendorBill() {
	...
	var shipmentRec = nlapiLoadRecord('customrecord_h5_shipment',shipmentId);
	var shipmentStatus = shipmentRec.getFieldValue('custrecord_h5_shipment_status');
	var isBilled = shipmentRec.getFieldValue('custrecord_h5_billed');
	var proNum = shipmentRec.getFieldValue('custrecord_h5_carrier_pro');
	if ( shipmentStatus == 3 && isBilled == 'F' && proNum != 'null'){
		var shipVen = shipmentRec.getFieldValue('custrecord_h5_carrier');
		var shipDate = shipmentRec.getFieldValue('custrecord_h5_ship_date');
		var shipCost = shipmentRec.getFieldValue('custrecord_h5_shipment_selcost');
		var shipSOParent = shipmentRec.getFieldValue('custrecord_h5_so_parent_id');
		//get shipment lines information for template - ajr
		...
		var totWeight = 0;
		var fil = new Array();
		fil[0] = new nlobjSearchFilter('custrecord_h5_shipment_parent', null, 'is', shipmentId);
		var col = new Array();
		...
		rec.setFieldValue('custbody_h5_shipment_link', shipmentId);
		rec.setFieldValue('custbody_h5_so_parent', shipSOParent);
		...
		shipmentRec.setFieldValue('custrecord_h5_billed', 'T');
...
function mapPop() {
	...
	var url = nlapiResolveURL('SUITELET','customscript_h5_map_render','customdeploy_h5_map_render');
	url += '&h5maporigin=' + reqId;
	var gmapTitle = 'h5 Shipment Tracker';
	nlExtOpenWindow(url, '', '800', '480', '', null, gmapTitle);
}

function AddEstRateButton(form){
	form.setScript('customscript_h5_rate_shipment_bttn');
	form.addButton('custpage_setrate', 'Create Shipment', 'h5ShipGenInit()');
	nlapiLogExecution('DEBUG', 'We added a button', 'function is set to estShipgen');
}

function h5EstCS(type, form){
	if (nlapiGetRecordId() != '' && nlapiGetRecordId() != null) {
		var currentRecordId = nlapiGetRecordId();
		var recType = nlapiGetRecordType();
...
function getSaaSRatesNow(batId){
	...
	//get Origin Info
	var origEnt = shipRec.getFieldValue('custrecord_h5_shipper');
	var origRec = nlapiLoadRecord('customer', origEnt);
	var origAddr1 = origRec.getFieldValue('shipaddr1');
	var origAddr2 = origRec.getFieldValue('shipaddr2');
	var origCity = origRec.getFieldValue('shipcity');
	...
	var destEnt = shipRec.getFieldValue('custrecord_h5_consignee');
...
function generateShippoRatePasses(batId){
	...
	for(x=0; x < carriers.length; x++){
		...
		//rateLine.setFieldValue('custrecord_h5_ratepass_shipment_id', recId);
		rateLine.setFieldValue('custrecord_h5_ratepass_id', x + 1);
		rateLine.setFieldValue('custrecord_h5_lane_name', 'Shippo API')
		rateLine.setFieldValue('custrecord_h5_service_level_desc', serviceLevel[x]);
		rateLine.setFieldValue('custrecord_h5_total_shipment_cost', shipRate[x]);
		rateLine.setFieldValue('custrecord_h5_transit_days', transDays[x]);
		rateLine.setFieldValue('custrecord_h5_ratepass_carrier', carriername[x]);
		nlapiSubmitRecord(rateLine);
...
function ShippoRetriever(request, response){
	...
	//get Origin Info
	var origEnt = shipRec.getFieldValue('custrecord_h5_shipper');
	//var origName = shipRec.getFieldText('custrecord_h5_shipper');
	...
	//get Destination Info
	var destEnt = shipRec.getFieldValue('custrecord_h5_consignee');
	//var destName = shipRec.getFieldText('custrecord_h5_consignee');
	var destRec = nlapiLoadRecord('customer', destEnt);
...
function getComboRates(){
	...
	//set returned rates
	parRec.setFieldValue('custbody_h5_parcel_cost', bestParcelRate);
	parRec.setFieldValue('custbody_h5_ltl_cost', bestLTLRate);
