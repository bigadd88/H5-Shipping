...
//end of testing variables
    ...
    nlapiLogExecution('debug', 'XML Returned from RL', respObj);
    var returnXml = nlapiStringToXML(respObj);
    var xmlbody = nlapiSelectNode(returnXml, "//*[name()='GetRateQuoteResult']");
    nlapiLogExecution('debug', 'xml body after query GetRateQuoteResult', xmlbody);
    var jsonbody = xmlToJson(xmlbody);
    nlapiLogExecution('debug', 'json Body after xmlToJson function(xmlBody)', jsonbody);
    var chargeType = [];
    var chargeTitle = [];
    var chargeWeight = [];
    var chargeRate = [];
    ...
    nlapiLogExecution('debug', 'show jsonBody data before fail', jsonbody);
    nlapiLogExecution('debug', 'failing right before serviceLevels.title', '');
...
function GetAccessorials(parentShipment) {
    ...
    nlapiLogExecution('debug', 'inside GetAccessorials Function');
    if (selAccessorials != ''){
