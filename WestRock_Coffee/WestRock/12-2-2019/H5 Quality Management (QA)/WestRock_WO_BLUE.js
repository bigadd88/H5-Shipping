function beforeLoad(type, form){
	...
	addGrinderButton(form);



}
function addGrinderButton(form){
	form.setScript('customscript_h5_wo_blue');
	form.addButton('custpage_grinder', 'H5-Grinder Placard', 'createGrinderPlacard()');
}
function createGrinderPlacard(){
	var recName = nlapiGetRecordId();
	var woRec = nlapiLoadRecord('workorder', recName);
	var lotPrefix = woRec.getFieldValue('custbody_h5_roasting_lot_naming_prefix');
	var pageCount = woRec.getFieldValue('custbody_h5_placardstoprint');
	var greenRecord = woRec.getFieldValue('custbody_wcr_green_coffee_lots');
if (lotPrefix == null) {
	alert('You are missing the roasting lot prefix');
}
if (pageCount == null) {
	alert('You are missing the placards to print quantity');
}
if (greenRecord == null) {
	alert('You are missing the green coffee lots');
}
	var url = nlapiResolveURL('SUITELET','customscript_h5_grinder_placard','customdeploy_h5_grinder_placard');
	url += '&recName=' + recName;
	window.open(url);
}
